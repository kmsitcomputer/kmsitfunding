...
const saveDraft = () => form.patch(`/admin/content/pages/${props.page.ulid}`);
const publish = () => publishForm.post(`/admin/content/pages/${props.page.ulid}/publish`);
const unpublish = () => unpublishForm.post(`/admin/content/pages/${props.page.ulid}/unpublish`);
const archive = () => {
    if (confirm(`Archive "${props.page.title}"? This cannot be undone.`)) {
        archiveForm.post(`/admin/content/pages/${props.page.ulid}/archive`);
    }
};
</script>
            ...
            <Card>
                ...
                <div class="mt-4 flex flex-wrap items-end gap-2">
                    <FormField label="Path" for="publish-path" :error="publishForm.errors.path" class="min-w-52 flex-1">
                        <Input id="publish-path" v-model="publishForm.path" placeholder="/about" />
                    </FormField>
                    <Button :disabled="publishForm.processing" @click="publish">Publish</Button>
                    <Button v-if="page.status === 'PUBLISHED'" variant="secondary" :disabled="unpublishForm.processing" @click="unpublish">Unpublish</Button>
                    <Button v-if="page.status === 'DRAFT' || page.status === 'RETIRED'" variant="danger" :disabled="archiveForm.processing" @click="archive">
                        Archive
                    </Button>
                </div>
            </Card>
